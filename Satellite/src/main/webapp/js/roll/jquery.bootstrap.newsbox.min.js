...
(function (e, t, n, r) {
	var i = {
		...
		onPrev : function () {
			...
			var cur = getNowFormatDate();
			var last = e(t.$elem).find(t.newsTagName).last().html();
			var tmp = last.split("|");
			...
			last = last + "time:"+cur;
			if(tmp[2].split(":")[1] == 1){
				last = last + "|<span style='color:green'>request</span>"
			}
			else
				last = last + "|<span style='color:red'>response</span>"
			var n = "<" + t.newsTagName + ' style="display:none;" class="' + t.newsClassName + '">' + last + "</" + t.newsTagName + ">";			
			
			e(t.$elem).prepend(n);
			var intervaltime = (Math.random())*10000;//delay
			
			$.ajax({
				type : "post",
				url : "datalog/saveDataLog",
				data:{"dataFormat":tmp[0].split(":")[1],"dataSize":tmp[1].split(":")[1],"type":tmp[2].split(":")[1],"reqCode"
				:"100002016080815336","status":tmp[3].split(":")[1],"time":cur,"dataType":"A"},
				success : function() {
					if(tmp[2].split(":")[1] == "2"){//收到数据
						var speed = (60/intervaltime*100000).toFixed(2);
						speed = speed + "KB/S";
						document.getElementById ("fileSpeed").innerHTML = speed;//update speed;
						
						var element = document.getElementById("table2_body");//update table2
						var num = tmp[1].split(":")[1];
						num = num.substring(0,num.length-5);
						num=parseInt(num)/20;
						myorder = myorder + num;
						update_table3(element,myorder);
					}
				},
				error : function() {
			...
			t.options.newsTickerInterval = intervaltime;
		...
		onNext : function () {
			//alert("onNext");
			/*var myli=$("#demo3 li:first");//获取第一个li的内容
			alert(myli.text());*/
			var t = this;
			if (t.animationStarted) {
				return false
